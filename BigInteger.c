...
//  Inserting a new element at the end of a BigInteger
void insert_tail(struct BigInteger *num, int ele) {
  struct node *temp = (struct node *)malloc(sizeof(struct node));
  if (temp == NULL) {
...
//  display a linked list in reverse order
...
//  initialize a BigInteger from a string
...
struct BigInteger mod(struct BigInteger num1, struct BigInteger num2) {
  ...
  struct BigInteger modul = sub(num1, temp2);
  num1.sign = t1;
  num2.sign = t2;
  return modul;
}
